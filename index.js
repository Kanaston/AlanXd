...
const final_table = document.querySelector("#tbF");
const formPesos = document.querySelector("#formPesos");
...
function res_arrows() {
  for (let i = 1; i < final_table.rows.length; i++) {
    let almacenar_info = [];
    for (let a = 0; a < final_table.rows[i].cells.length; a++) {
      almacenar_info.push(Number(final_table.rows[i].cells[a].innerHTML));
    }

    createCell(
      final_table.rows[i].insertCell(final_table.rows[i].cells.length),
      promedio(almacenar_info),
      "col"
    );
  }
}
...
function promedio(arreglo) {
  ...
  return sum_total / arreglo.length;
}
          ...
          if (a.Columna === index) {
            if (typeof a.Dato === "string") {
              if (a.Columna === 3 || a.Columna === 5 || a.Columna === 6) {
                a.Dato.match(/[0-9]+/g).map(
                  (number) => (a.Dato = Number(number))
                );
              }
...
const getFormValues = () => {
  let retArr = [];
  for (let i = 0; i < formPesos.length - 1; i++) {
    if (formPesos[i].value) {
      retArr.push(Number(formPesos[i].value));
    } else {
      retArr.push(0);
    }
  }
...
const refreshTable = () => {
  ...
  const valid = formValues.filter(num => num<=0)
const validSum = formValues.reduce((total,value)=>total+=value)
  if(valid>=0 && validSum>100){
    
    for (let i = 1; i < 11; i++) {
      let formResult = [];
      for (let j = 0; j < 10; j++) {
        const cell = final_table.rows[i].cells[j];
        formResult.push((formValues[j] / 100) * Number(cell.innerHTML));
      }
      console.log(promedio(formResult));
      final_table.rows[i].cells[10].innerHTML = promedio(
        formResult
      )*10;
    }
  }
